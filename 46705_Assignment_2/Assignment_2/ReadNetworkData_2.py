...
def parse_transmission_line_data(row_):
    # unpack the values:    
    fr,to,br_id,R,X,B_half,X2,X0, MVA_rate = row_[0:9]
...
#    br_id = br_id[1:-1]     #get the id out of the string
    ...
    return [fr,to,br_id,R,X,B_half,X2,X0, MVA_rate]
...
def parse_transformer_data(row_):
    # unpack the values:
    fr,to,br_id,R,X,n,ang1,fr_co,to_co,X2,X0, MVA_rate = row_[0:12]
...
#    br_id = br_id[1:-1]     #get the id out of the string
    ...
    X0 = float(X2)
    MVA_rate=float(MVA_rate)
    return [fr,to,br_id,R,X,n,ang1,fr_co,to_co,X2,X0,MVA_rate] 
